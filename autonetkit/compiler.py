    ...
    def bgp(self, node):
        ...
        for session in G_bgp.edges(phy_node):
            ...
            if session.type == "ibgp":
                ...
                print "session", session, session.src.id, session.dst.id, neigh.id
                print "ip link", ip_link.src.id, ip_link.dst.id, neigh.id
                print "edges", list(G_ip.edges(ip_link.dst))
                print "edges", list(G_ip.edges(ip_link.dst, neigh))
