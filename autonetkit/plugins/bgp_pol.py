from pyparsing import Suppress, Word, ZeroOrMore, alphas, nums, Literal, Group, Optional, Forward
import json
...
# TODO: drop pol_clause.map, etc and use as children: or have visitor that
# returns these as children - easier to traverse tree
...
# TODO: make methods that atoms inherit: key, value eg (setLP, 120),
# (prefix_list, blah_abc)
...
def fn_if(strg, loc, toks):
    # Extract matches and actions - need to search through list. Return first
    # (only) instance
...
class pol_set_lp(pol_action_atom):
    # TODO: take either an integer value, or if alpha, then need to add it to
    # a list to perform ordering on... may be better to have two different
    # actions, and then feed into the same class once processed.
...
# tags contain xyz
match_tags = Group(
    Literal("tags contain") + Word(alphas)).setParseAction(fn_match_tags)
match_pl = Group(
    Literal("prefix_list") + is_token + Word(alphas)).setParseAction(fn_match_pl)
...
# setMED med
token_set_med = Group(
    Literal("setMED") + Word(nums)).setParseAction(fn_set_med)
# addTag tag
token_add_tag = Group(
    Literal("addTag") + Word(nums)).setParseAction(fn_add_tag)
# removeTag tag
token_remove_tag = Group(
    Literal("removeTag") + Word(nums)).setParseAction(fn_remove_tag)
# reject
...
action_clause = (
    token_set_lp | token_set_med | token_add_tag | token_remove_tag | token_reject)
...
else_actions = (
    action_clause + ZeroOrMore(and_token + action_clause)) | bgp_pol
else_clause = Group(Suppress("else") + Suppress("(") + else_actions
                    + Suppress(")")).setParseAction(fn_else)

bgp_pol << Group(Group(Suppress("if") + match_clauses + then_clause +
                       Optional(else_clause)) | standlone_then_clause).setParseAction(fn_if)
...
# also need to walk tree to build up a list of all tags present, and all
# prefix lists

# end up with lists of named tuples for tokens: (match, value), and
# (action, value) that can then format inside the template - that way
# slight differences between syntaxes are able to be handles in templates
# rather than back up in the parser
...
# TODO: could this be done by a variation of the BgpPolEncoder visitor, in
# that any instances of prefix_list or tag types are returned recursively
# and then flattened at the end?
