...
# Load these templates
    ...
    for node in G_out:
        ...
        # create copy to append with
        v_properties = dict(templates[template].node[v])
    ...
    nx.relabel_nodes(G_out, dict(((u, v), "%s_%s" % (v, u))
                                 for (u, v) in G_out), copy=False)
...
# Node lists - if 'root' set then only use root nodes
        N1 = [n for n, d in H1.nodes(
            data=True) if 'interpop' in d and d['interpop']]
        if not len(N1):
            N1 = [n for n in H1]  # no nodes marked interpop

        N2 = [n for n, d in H2.nodes(
            data=True) if 'interpop' in d and d['interpop']]
        if not len(N2):
            N2 = [n for n in H2]  # no nodes marked interpop

        log.debug("Adding edges for (%s,%s) with operator %s" %
                  (u1, u2, operator))

        log.debug("H nodes for u1 %s: %s" %
                  (G.node[u1]['pop_template'], ", ".join(str(N1))))
        log.debug("H nodes for u2 %s: %s" %
                  (G.node[u2]['pop_template'], ", ".join(str(N2))))
# 'root' not set
# TODO: fold rooted back into special case of cartesian - just do the same
# for now
        if operator == 'rooted':
            product_edges = [((u1, v1), (u2, v2)) for v1 in N1 for v2 in N2
                             if H1.node[v1].get("interpop") == H2.node[v2].get("interpop") == True]
            log.debug("Rooted product edges for (%s,%s): %s" %
                      (u1, u2, product_edges))
            edges += product_edges

        if operator == 'lexical':
            product_edges = [((u1, v1), (u2, v2)) for v1 in N1 for v2 in N2]
            log.debug("Lexical product edges for (%s,%s): %s" %
                      (u1, u2, product_edges))
            edges += product_edges

        if operator in cartesian_operators:
            product_edges = [((u1, v1), (u2, v2))
                             for v1 in N1 for v2 in N2 if v1 == v2]
            log.debug("Cartesian product edges for (%s,%s): %s" %
                      (u1, u2, product_edges))
            edges += product_edges
        if operator in tensor_operators:
            product_edges = [((u1, v1), (u2, v2)) for v1 in N1 for v2 in N2
                             if H1.has_edge(v1, v2) or H2.has_edge(v1, v2)]
            log.debug("Tensor product edges for (%s,%s): %s" %
                      (u1, u2, product_edges))
            edges += product_edges
